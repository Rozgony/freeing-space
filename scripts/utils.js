...
function hideBackdrop(){
	$('.leaflet-marker-icon, .leaflet-interactive').addClass('fade-in');
...
function createPolygonGeometry(geom){
	const openIndex = geom.indexOf('(') + 2;
	const closeIndex = geom.lastIndexOf(')');
	const coordinates = geom.slice(openIndex,closeIndex).split(',').map( point => {
		const openIndex = point.indexOf('(') + 1;
		const closeIndex = point.lastIndexOf(')') - 2;
		return point.slice(openIndex,closeIndex).split(' ').map( value => {
...
function createMultiPolygonGeometry(geom){
	...
	function getNextChunk(geoString){
		...
		const coordsGroup = group.trim().split(',').map( coords => {
			return coords.trim().split(' ').map( value => {

...
function generateHtmlContent(menuItems)
{
    ...
    for (var i = 0; i < menuItems.Items.length;i++)
    {
        ...
        if (item.type == 'link') {
            ...
            content += `<span class=\"panel-list-item-icon\"><img class="menu-icon plus" src="${item.icon}"/></span>`;
            content += '<a class="link" href=\"' + item.href + '\" target="_blank">' + item.name + '</a>';
        }
        else if (item.type == 'button') {
            // content += '<span class=\"panel-list-item-icon ' + item.icon + '\" ></span>';
            // content += '<span class=\"panel-list-item-icon material-icons\">'  + item.icon+ '</span>';
            content += `<span class=\"panel-list-item-icon\"><img class="menu-icon" src="${item.icon}"/></span>`;
